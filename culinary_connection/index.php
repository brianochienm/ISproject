...
<head>
    ...
    <title>Culinary Connection - Administrator Homepage</title>
        ...
        <nav id="tm-nav" class="fixed w-full">
            <div class="tm-container mx-auto px-2 md:py-6 text-right">
                ...
                <ul class="mb-3 md:mb-0 text-2xl font-normal flex justify-end flex-col md:flex-row">
                    ...
                    <li class="inline-block mb-4 mx-4"><a href="#module" class="tm-text-gold py-1 md:py-3 px-4">Administrator Module</a></li>
                </ul>
            </div>            
        </nav>
            ...
            <div class="sm:pb-60 sm:pt-48 py-20">
                ...
                <div class="bg-black bg-opacity-70 p-10 mb-5">
                    <p class="text-white leading-8 text-sm font-light">
                     ...
                     <a rel="nofollow" href="#module" target="_parent">Administrator Module</a>! </p>
                </div>
    ...
    <div id="data" class="parallax-window" data-parallax="scroll" data-image-src="img/b2.jpg">
        <div class="container mx-auto tm-container py-24 sm:py-48">
                <div class="flex-1 rounded-xl p-12 pb-14 m-5 bg-black bg-opacity-50 tm-item-container">
                                      <h2 class="mb-6 tm-text-green text-4xl font-medium">List of Users</h2>
                                      <br>
                                           <table id="printTable">
...
<?php
$sql = "SELECT `user_id`, `fullname`, `phone_number`, `email`, `role`, `country` FROM `tbl_users`";
...
<br>
<br>
                <a onclick="printData();" class="inline-block tm-bg-green transition text-white text-xl pt-3 pb-4 px-8 rounded-md">
                    Print Report
                </a> 
...
<?php
$sql = "SELECT `Market_ID`, `Provider_Name`, `Item_Name`, `Item_Type`, `Image`, `Created_At`, `Expire_At`, `Price`, `Quantity` FROM `market`";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<td><img src="img/<?php echo($row["Image"]); ?>" style="width: 150px;" title="<?php echo($row["Item_Name"]); ?>"></td>
...
<?php
$sql = "SELECT `orders`.`Order_ID`, `orders`.`Provider_Name`, `orders`.`Item_Name`, `Item_Type`, `orders`.`Buyer_Name`, `orders`.`Created_At`, `orders`.`Price`, `orders`.`Quantity`, `orders`.`Status`, `orders`.`Payment_Means`, `tbl_users`.`phone_number`, `tbl_users`.`email` FROM `orders` JOIN `tbl_users` ON `tbl_users`.`fullname` = `orders`.`Provider_Name`";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
            ...
            <div class="flex flex-col lg:flex-row justify-around items-center lg:items-stretch">
                ...
                <div class="flex-1 rounded-xl p-12 pb-14 m-5 bg-black bg-opacity-50 tm-item-container">
                    <form action="insertion.inc.php" method="POST" class="text-lg">
                        ...
                        <input type="text" name="phone" class="input w-full bg-black border-b bg-opacity-0 text-white px-0 py-4 mb-4 tm-border-gold" placeholder="Phone Number" required="" />
                        <input type="email" name="email" class="input w-full bg-black border-b bg-opacity-0 text-white px-0 py-4 mb-4 tm-border-gold" placeholder="Email" required="" />
                        <input type="text" name="country" class="input w-full bg-black border-b bg-opacity-0 text-white px-0 py-4 mb-4 tm-border-gold" placeholder="Country" required="" />
                     <select required name="role" class="input w-full bg-black border-b bg-opacity-0 text-white px-0 py-4 mb-4 tm-border-gold">
                        <option selected disabled value="0">Select A Role</option>
                        <option value="Administrator">Administrator</option>
                        <option value="Culinary Providers">Culinary Providers</option>
                        </select>
                        <input type="password" name="password" minlength="8" class="input w-full bg-black border-b bg-opacity-0 text-white px-0 py-4 mb-4 tm-border-gold" placeholder="Password" required="" />
                        <input type="password" name="cpassword" minlength="8" class="input w-full bg-black border-b bg-opacity-0 text-white px-0 py-4 mb-4 tm-border-gold" placeholder="Confirm Password" required="" />
                        <div class="text-right">
                            <button type="submit" class="text-white hover:text-yellow-500 transition" name="regu">Register</button>
                        </div>                        
                      </form>
                </div>
            </div>
